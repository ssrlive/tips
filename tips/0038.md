...
## 给公网 IP 请求 Let's Encrypt 证书

在 2026 年 1 月， [Let's Encrypt](https://letsencrypt.org/2026/01/15/6day-and-ip-general-availability)
宣布 开放 对 纯公网 IP 颁发证书， 我写了一个脚本 [pure-ip-cert.sh](pure-ip-cert.sh) 来完成这件事.

这个脚本主要调用 [acme.sh](https://github.com/acmesh-official/acme.sh) 完成整个操作。
只要你当前的 位于公网的 IP 的 ubuntu 主机上， 运行下面的命令就可以申请到证书了。
...
这个脚本还生成一个位于 `~/.acme.sh/${hostaddr}_ecc` 目录下名为 `renew_cert.sh` 的脚本，
它会被加入到 `crontab` 里， 每 2 天运行一次， 以确保证书在过期前及时续签。

## 在 ubuntu 内让 某指定目录 的读写权限 同时指派给 当前账号 和 另一账号

我们编写网站程序时，经常得访问 `/var/www/html` 目录， 为方便 `当前账号` 和 `www-data`
账号同时读写这个目录， 我写了个 [脚本](share_dir.sh) 来做这件事。
只需传入 `目录路径` 和 `对方账号`。例如，你想让你和 `www-data` 共享 `/var/www/html` 目录,
执行 `./share_dir.sh /var/www/html www-data` 命令就成了。

```bash
curl -L https://github.com/ssrlive/tips/raw/refs/heads/master/tips/share_dir.sh -o share_dir.sh
bash ./share_dir.sh /var/www/html www-data
```

## 在 Linux Mint 中安装最新的 Fcitx5 输入法框架

https://ivonblog.com/posts/install-fcitx5-on-linux-mint/
