...
## 在 Windows 下后台隐藏执行命令行程序不出现黑窗口
```powershell
Start-Process -FilePath "youapp.exe" -ArgumentList "--config path/of/cfg.toml" -WindowStyle Hidden
```
...
- 運行 msconfig 命令後，重啓系統，這時候一定會藍屏死機（BSOD），如此低級的錯誤竟然堂而皇之地無人修復，無處報告。啊呸！
