...
1. 到 [wxWidgets 官網](https://www.wxwidgets.org/downloads/) 下載最新版本的 wxWidgets。
   我這裏下載到的是 wxWidgets-3.3.1.zip。

2. 創建一個目錄 `wxMyApp`，然後將下載的 `wxWidgets-3.3.1.zip` 解壓到這個目錄下。
...
4. 在 `wxMyApp` 目錄下創建一個名為 `CMakeLists.txt` 的文件，內容如下：
   ...
   if(CMAKE_SYSTEM_NAME MATCHES Darwin)
       set(COMPILE_OPTIONS "MACOSX_BUNDLE")
   elseif(CMAKE_SYSTEM_NAME MATCHES Windows)
       set(COMPILE_OPTIONS "WIN32")
   elseif(CMAKE_SYSTEM_NAME MATCHES Linux)
       set(COMPILE_OPTIONS "")
   else()
       message(FATAL_ERROR "Unsupported platform: ${CMAKE_SYSTEM_NAME}")
   endif()
   add_executable(wxMyApp ${COMPILE_OPTIONS} main.cpp)

   target_link_libraries(wxMyApp PRIVATE wx::base)

   if(wxUSE_GUI)
       target_link_libraries(wxMyApp PRIVATE wx::core wx::adv)
   endif()

   if(wxUSE_AUI)
       target_link_libraries(wxMyApp PRIVATE wx::aui)
   endif()

   if(wxUSE_OPENGL)
       target_link_libraries(wxMyApp PRIVATE wx::gl)
   endif()

   if(wxUSE_HTML)
       target_link_libraries(wxMyApp PRIVATE wx::html)
   endif()

   if(wxUSE_MEDIACTRL)
       target_link_libraries(wxMyApp PRIVATE wx::media)
   endif()

   if(wxUSE_SOCKETS)
       target_link_libraries(wxMyApp PRIVATE wx::net)
   endif()

   if(wxUSE_PROPGRID)
       target_link_libraries(wxMyApp PRIVATE wx::propgrid)
   endif()

   if(wxUSE_DEBUGREPORT)
       target_link_libraries(wxMyApp PRIVATE wx::qa)
   endif()

   if(wxUSE_RIBBON)
       target_link_libraries(wxMyApp PRIVATE wx::ribbon)
   endif()

   if(wxUSE_RICHTEXT)
       target_link_libraries(wxMyApp PRIVATE wx::richtext)
   endif()

   if(wxUSE_STC)
       target_link_libraries(wxMyApp PRIVATE wx::stc)
   endif()

   if(wxUSE_WEBVIEW)
       target_link_libraries(wxMyApp PRIVATE wx::webview)
   endif()

   if(wxUSE_XML)
       target_link_libraries(wxMyApp PRIVATE wx::xml)
   endif()

   if(wxUSE_XRC)
       target_link_libraries(wxMyApp PRIVATE wx::xrc)
   endif()
   ...
   # build command:
   # ```bash
   # cmake . -B ./build_dir -G Ninja && cmake --build ./build_dir
   # ```
   ```

5. 在 `wxMyApp` 目錄下打開終端，執行以下兩條命令， 數分鐘後將生成最後結果 `build_dir/wxMyApp` 可執行文件：
...
6. 執行 `build_dir/wxMyApp`，應該會看到一個簡單的窗口，附帶菜單和狀態欄。
